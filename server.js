...
// API: Launch an agent for an issue (reintroduced with improved credential handling)
app.post('/api/launch-agent', (req, res) => {
    ...
    // Debug credentials to help troubleshoot
    console.log('Checking credentials existence:');
    try {
        const hasMainConfig = fs.existsSync(`${process.env.HOME}/.claude.json`);
        const hasCredentials = fs.existsSync(`${process.env.HOME}/.claude/.credentials.json`);
        console.log(`- Main config (~/.claude.json): ${hasMainConfig ? 'exists' : 'missing'}`);
        console.log(`- Credentials (~/.claude/.credentials.json): ${hasCredentials ? 'exists' : 'missing'}`);
    } catch (err) {
        console.log('❌ Error checking credential files:', err.message);
    }
